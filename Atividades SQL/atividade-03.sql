...
-- 4.  Recupere os detalhes de uma compra específica, incluindo o nome do cliente, a data da compra e o total.
	...
	select clientes.nome, compras.data_compra, compras.total_compra from clientes inner join compras on clientes.id = compras.cliente_id where compras.id = 2; 

-- 5.  Liste os carros que estão em estoque, juntamente com a quantidade disponível.

	select nome, quantidade_estoque from carros 
    where quantidade_estoque > 0;

-- 6.  Crie uma consulta para mostrar os clientes que compraram um determinado carro (usando o ID do carro).

	select clientes.nome from 
    clientes inner join compras on clientes.id = compras.cliente_id 
    inner join carros_compra on compras.id = carros_compra.compra_id 
    where carros_compra.carro_id = 1 ;
    
-- 6.2  Crie uma consulta para mostrar os clientes e o nome do carro que compraram um determinado carro (usando o ID do carro).
    
    select clientes.nome, carros.nome from 
    clientes inner join compras on clientes.id = compras.cliente_id 
    inner join carros_compra on compras.id = carros_compra.compra_id 
    inner join carros on carros_compra.carro_id = carros.id
    where carros_compra.carro_id = 1 ;
    
-- 7.  Exiba o total de cada compra, juntamente com os nomes dos carros comprados, para um cliente específico.

	select compras.total_compra, carros.nome from 
    clientes inner join compras on clientes.id = compras.cliente_id 
    inner join carros_compra on compras.id = carros_compra.compra_id
    inner join carros on carros_compra.carro_id = carros.id
    where clientes.id = 1;
...
-- 8.  Mostre os carros que foram comprados em uma determinada data.
	...
	select carros.nome from 
    carros inner join compras on carros.id = compras.id 
    where data_compra = "1989-12-13";

-- 9.  Recupere o total de vendas de cada cliente (nome e total) em ordem decrescente.

	select clientes.nome, compras.total_compra from
    clientes join compras on clientes.id = compras.cliente_id 
    order by compras.total_compra desc;

-- 10.  Exiba o nome do cliente que fez a compra mais cara até o momento.

	select clientes.nome, compras.total_compra 
    from clientes 
